...
async def send_changes(bot: commands.Bot, messages: list):
    channel_ids = await helpers.load_CSR2_announcement_channels()
    for channel_id in channel_ids:
        try:
            ...
            announce_updates.delete_channel(channel_id, scope="CSR2")
        except Exception as e:
            logging.error(f"Error while trying to send changes to {channel_id}: {e}")
    user_ids = await helpers.load_CSR2_announcement_users()
    for user_id in user_ids:
        try:
            ...
            notify_updates.delete_user(user_id, scope="CSR2")
        except discord.NotFound as e:
            notify_updates.delete_user(user_id, scope="CSR2")
        except Exception as e:
