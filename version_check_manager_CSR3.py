...
async def version_check_task(bot: commands.Bot):
    ...
    logging.info("CSR3 - Starting CSR3 version check")
    new_data = await fetch_data()
    logging.info("CSR3 - Comparing data")
    changes, new_data, case = await detect_changes(previous_data, new_data)

    logging.info("CSR3 - Overwriting old data with the new data")
    await save_data(new_data)

    if changes:
        logging.info("CSR3 - Sending detected changes")
