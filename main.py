...
async def on_ready():
    log = ""
    header = localisation.get('BOOT_LOG_HEADER')
    logger.info(f"{header}{localisation.get('BOOT_LOG_BOT_LOGIN')} {bot.user.name} ({bot.user.id})")
    log += f"{header}{localisation.get('BOOT_LOG_BOT_LOGIN')} {bot.user.name} ({bot.user.id})"
    status = 2
    ...
    if not schedule_db_updates.is_running():
        logger.info(f"{header}{localisation.get('BOOT_LOG_EDB_TASK_SCHEDULE_START')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_EDB_TASK_SCHEDULE_START')}"
        schedule_db_updates.start()
    if not schedule_version_check.is_running():
        logger.info(f"{header}{localisation.get('BOOT_LOG_VERSION_CHECK_TASK_SCHEDULE_START')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_VERSION_CHECK_TASK_SCHEDULE_START')}"
        schedule_version_check.start()
    if not schedule_blog_check.is_running():
        logger.info(f"{header}{localisation.get('BOOT_LOG_BLOG_TASK_SCHEDULE_START')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_BLOG_TASK_SCHEDULE_START')}"
        schedule_blog_check.start()
    if not schedule_profile_update.is_running():
        logger.info(f"{header}{localisation.get('BOOT_LOG_IDENTITY_UPDATE_TASK_SCHEDULE_START')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_IDENTITY_UPDATE_TASK_SCHEDULE_START')}"
        schedule_profile_update.start()
    if not schedule_dynamic_status_change.is_running():
        logger.info(f"{header}{localisation.get('BOOT_LOG_DYNAMIC_STATUS_TASK_SCHEDULE_START')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_DYNAMIC_STATUS_TASK_SCHEDULE_START')}"
    ...
    try:
        ...
        for server in ADMIN_SERVERS:
            ...
            logger.info(f"{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_START')} {admin_guild.name}")
            log += f"\n{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_START')} {admin_guild.name}"
            await bot.tree.sync(guild=discord.Object(id=int(server)))
            logger.info(f"{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_DONE')} {admin_guild.name}")
            log += f"\n{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_DONE')} {admin_guild.name}"
    except Exception as e:
        logger.error(f"{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_FAIL')} {e}")
        log += f"\n{header}{localisation.get('BOOT_LOG_SYNC_CMD_ADMIN_SERVER_FAIL')} {e}"
        status = 0

    FIRST_SETUP_DONE = await helpers.load_json_key("config", "FirstSetupDone")

    if not FIRST_SETUP_DONE:
        logger.info(f"{header}{localisation.get('BOOT_LOG_FIRST_SETUP_NOTICE')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_FIRST_SETUP_NOTICE')}"
        for file in os.listdir(f'{os.path.dirname(os.path.abspath(__file__))}/bot-emojis'):
            if file.endswith((".png", ".gif")):
                try:
                    ...
                    logger.info(f"{header}{localisation.get('BOOT_LOG_FIRST_SETUP_EMOJI_UPLOAD_DONE')} {emoji.name} ({emoji.id})")
                    log += f"\n{header}{localisation.get('BOOT_LOG_FIRST_SETUP_EMOJI_UPLOAD_DONE')} {emoji.name} ({emoji.id})"
                except Exception as e:
                    logger.error(f"{header}{emoji_name} {localisation.get('BOOT_LOG_FIRST_SETUP_EMOJI_UPLOAD_FAIL')} {e}")
                    log += f"\n{header}{emoji_name} {localisation.get('BOOT_LOG_FIRST_SETUP_EMOJI_UPLOAD_FAIL')} {str(e)[:35]}"
                    status = 1
    ...
    session_variables = {}
    logger.info(f"{header}{localisation.get('BOOT_LOG_SESSION_VAR_NOTICE')}")
    log += f"\n{header}{localisation.get('BOOT_LOG_SESSION_VAR_NOTICE')}"
    logger.info(f"{header}{localisation.get('BOOT_LOG_SESSION_VAR_FETCH_CMD')}")
    log += f"\n{header}{localisation.get('BOOT_LOG_SESSION_VAR_FETCH_CMD')}"
    ...
    logger.info(f"{header}{localisation.get('BOOT_LOG_SESSION_VAR_FETCH_EMOJI')}")
    log += f"\n{header}{localisation.get('BOOT_LOG_SESSION_VAR_FETCH_EMOJI')}"
    ...
    logger.info(f"{header}{localisation.get('BOOT_LOG_SESSION_VAR_SAVED')}")
    log += f"\n{header}{localisation.get('BOOT_LOG_SESSION_VAR_SAVED')}"
    ...
    if await helpers.load_json_key("status", "IsStaticStatusActive"):
        ...
        logger.info(f"{header}{localisation.get('BOOT_LOG_PRESSENCE_DONE')}")
        log += f"\n{header}{localisation.get('BOOT_LOG_PRESSENCE_DONE')}"
...
async def update_bot_profile(pfp_path, new_name):
    log = ""
    header = localisation.get('IDENTITY_UPDATE_LOG_HEADER')
    logger.info(f"{header}{localisation.get('IDENTITY_UPDATE_LOG_START')}")
    log += f"{header}{localisation.get('IDENTITY_UPDATE_LOG_START')}"
    async with aiofiles.open(pfp_path, mode='rb') as pfp_file:
        bot_pfp = bot.user.avatar
        if await bot_pfp.to_file() != pfp_file:
            try:
                await bot.user.edit(username=new_name, avatar=await pfp_file.read())
                logger.info(f"{header}{localisation.get('IDENTITY_UPDATE_LOG_DONE')}")
                log += f"\n{header}{localisation.get('IDENTITY_UPDATE_LOG_DONE')}"
            except discord.HTTPException as e:
                logger.info(f"{header}{localisation.get('IDENTITY_UPDATE_LOG_FAIL')} {e}")
                log += f"\n{header}{localisation.get('IDENTITY_UPDATE_LOG_FAIL')} {e}"
...
async def schedule_profile_update():
    try:
        ...
        logger.error(f"{localisation.get('IDENTITY_UPDATE_LOG_HEADER')}{localisation.get('IDENTITY_UPDATE_LOG_FAIL')} {e}")
        log = f"{localisation.get('IDENTITY_UPDATE_LOG_HEADER')}{localisation.get('IDENTITY_UPDATE_LOG_FAIL')} {e}"
        await in_app_logging.send_log(bot, log, 0, 2)
...
async def schedule_db_updates():
    ...
    try:
        ...
        logger.error(f"{header}{localisation.get('EDB_LOG_UPDATE_FAIL')} {e}")
        log = f"{header}{localisation.get('EDB_LOG_UPDATE_FAIL')} {e}"
        await in_app_logging.send_log(bot, log, 0, 2)


@tasks.loop(minutes=1)
async def schedule_version_check():
    header = localisation.get('VERSIONCHECK_LOG_HEADER')
    try:
        await version_check_manager_apps.version_check_task(bot)
        await asyncio.sleep(0.0)
    except Exception as e:
        logger.error(f"{header}{localisation.get('VERSION_CHECK_LOG_FAIL')} {e}")
        log = f"{header}{localisation.get('VERSION_CHECK_LOG_FAIL')} {e}"
        await in_app_logging.send_log(bot, log, 0, 2)
...
async def schedule_blog_check():
    try:
        ...
        logger.error(f"{localisation.get('IDENTITY_UPDATE_LOG_HEADER')}"
                     f"{localisation.get('BLOG_LOG_UPDATE_CHECK_FAIL')} {e}")
        log = (f"{localisation.get('IDENTITY_UPDATE_LOG_HEADER')}"
               f"{localisation.get('BLOG_LOG_UPDATE_CHECK_FAIL')} {e}")
        await in_app_logging.send_log(bot, log, 0, 2)
...
async def schedule_dynamic_status_change(bot: commands.Bot):
    try:
        ...
        logger.error(f"{localisation.get('DYNAMIC_STATUS_LOG_HEADER')}"
                     f"{localisation.get('DYNAMIC_STATUS_LOG_FAIL')} {e}")
        log = (f"{localisation.get('DYNAMIC_STATUS_LOG_HEADER')}"
               f"{localisation.get('DYNAMIC_STATUS_LOG_FAIL')} {e}")
        await in_app_logging.send_log(bot, log, 0, 2)
