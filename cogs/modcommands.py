...
class ModcommandsCog(commands.Cog):
    ...
    async def modcommands(self, interaction: discord.Interaction):
        # Log the command usage and parameters
        logger.info(f"The following command has been used: /csr2_modcommands")
        log = f"The following command has been used: /csr2_modcommands"
        
        if interaction.user.guild_permissions.administrator or str(interaction.user.id) == str(NITRO):
            await interaction.response.defer()

            embed = discord.Embed(
                title="Available Moderator Commands",
                description=(
                    f"## </csr2_limitresults:1266755136114659370>\nAdditional operators:\n- limit: Accepts integers.\n"
                ),
