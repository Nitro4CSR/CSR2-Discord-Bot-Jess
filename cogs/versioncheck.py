...
class VersionCheckCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        @app_commands.command(name=self.bot.localisation.get('VERSION_CHECK_CMD_NAME'), description=self.bot.localisation.get('VERSION_CHECK_CMD_DESC'))
        @app_commands.describe(app=self.bot.localisation.get('VERSION_CHECK_CMD_APP'), store=self.bot.localisation.get('VERSION_CHECK_CMD_STORE'), version=self.bot.localisation.get('VERSION_CHECK_CMD_VERSION'), country=self.bot.localisation.get('VERSION_CHECK_CMD_COUNTRY'))
        @app_commands.choices(app=[app_commands.Choice(name=self.bot.localisation.get('VERSION_CHECK_CMD_APP_OPTION_CSR2'), value="CSR2"), app_commands.Choice(name=self.bot.localisation.get('VERSION_CHECK_CMD_APP_OPTION_CSR3'), value="CSR3")])
        @app_commands.choices(store=[app_commands.Choice(name=self.bot.localisation.get('VERSION_CHECK_CMD_STORE_OPTION_GP'), value="Google Play"), app_commands.Choice(name=self.bot.localisation.get('VERSION_CHECK_CMD_STORE_OPTION_AS'), value="App Store")])
        async def versioncheck(interaction: discord.Interaction, app: str = None, store: str = None, version: str = None, country: str = None):
            await interaction.response.defer()
            try:
                header = self.bot.localisation.get('VERSION_CHECK_LOG_HEADER')
                logger.info(f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('VERSIONCHECK_CMD_NAME')} app: {app} store: {store} version: {version} country: {country}")
                log = f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('VERSIONCHECK_CMD_NAME')} app: {app} store: {store} version: {version} country: {country}"
                data_sources = []
                ...
                logger.info(f"{header}{len(grouped_results)} {self.bot.localisation.get('LOG_RESULTS_FOUND')}")
                log += f"\n{header}{len(grouped_results)} {self.bot.localisation.get('LOG_RESULTS_FOUND')}."
                batch = []
                embeds = []
                ...
                if grouped_results:
                    for result in grouped_results:
                        ...
                        len(continent_info)
                        embed = discord.Embed(
                            title=f"{self.bot.localisation.get('VERSION_CHECK_MSG_EMBED_TITLE')}",
                            description=(
                                f"## {self.bot.localisation.get('VERSION_CHECK_MSG_EMBED_DESC_STORE')} {result['store']}\n{self.bot.localisation.get('VERSION_CHECK_MSG_EMBED_DESC_VERSION')} {result['version']}\n{self.bot.localisation.get('VERSION_CHECK_MSG_EMBED_DESC_LAST_UPDATED')} <t:{result['last_updated']}:F>\n\n{continent_info}"
                            ),
                    ...
                    embed = discord.Embed(
                        title=f"{self.bot.localisation.get('VERSION_CHECK_MSG_EMBED_TITLE_NO_RESULTS')}",
                        description="",
                        color=discord.Color(0xFF00FF),
                    )
                    embeds.append(embed)
        ...
        self.bot.tree.add_command(versioncheck)
