...
class VisionCommandCog(commands.Cog):
    ...
    @app_commands.command(name=localisation.get('VISION_CMD_NAME'), description=localisation.get('VISION_CMD_DESC'))
    ...
    async def vision(self, interaction: discord.Interaction, car: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
        await interaction.response.defer()
        try:
            header = localisation.get('VISION_LOG_HEADER')
            logger.info(f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('VISION_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}")
            log = f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('VISION_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}"
    ...
    async def create_embeds(self, results: list, log: str):
        ...
        for row in results:
            ...
            embed.add_field(name=f"{localisation.get('VISION_MSG_EMBED_DESC_S6_EFFECTS')}", value=value)
