...
class S6ECog(commands.Cog):
    ...
    def construct_results(self, rows: list, log: str):
        ...
        for row in rows:
            ...
            embed.add_field(name=f"Tier: {row[3]}", value=f"\n**S5-PP: {row[5]}**", inline=True)
            embed.add_field(name=f"Stars: {row[4]}", value=f"**S5-EVO: {row[6]}**", inline=True)
            embed.add_field(name="", value="", inline=True)
            embed.add_field(name=f"S5-Nos: {row[7]}", value=f"**S5-Dyno: {float(row[10]):.3f}**", inline=True)
