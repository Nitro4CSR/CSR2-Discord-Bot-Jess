...
class WRCommandCog(commands.Cog):
    ...
    def construct_results(self, rows: list, log: str):
        ...
        for row in rows:
            ...
            embed.add_field(name=f"Tier: {row[3]}", value=f"**Stars: {row[4]}**", inline=True)
            embed.add_field(name=f"PP: {row[5]}", value=f"**EVO: {row[6]}**", inline=True)
            ...
            embed.add_field(name=f"Dyno: {float(row[10]):.3f}", value=f"")
