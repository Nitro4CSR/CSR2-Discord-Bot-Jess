...
class WRCommandCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        @app_commands.command(name=self.bot.localisation.get('WR_CMD_NAME'), description=self.bot.localisation.get('WR_CMD_DESC'))
        ...
        async def wr(interaction: discord.Interaction, car: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
            await interaction.response.defer()
            try:
                header = self.bot.localisation.get('WR_LOG_HEADER')
                logger.info(f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('WR_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}")
                log = f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('WR_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}"
    ...
    async def create_embeds(self, results: list, log: str):
        ...
        for row in results:
            ...
            embed.add_field(name=f"{self.bot.localisation.get('WR_MSG_EMBED_DESC_PP')} {result[5]}\n{self.bot.localisation.get('WR_MSG_EMBED_DESC_EVO')} {result[6]}\n", value=f"\n", inline=False)
            embed.add_field(name=f"{self.bot.localisation.get('WR_MSG_EMBED_DESC_NOS')} {result[7]}\n{self.bot.localisation.get('WR_MSG_EMBED_DESC_FD')} {result[8]}\n{self.bot.localisation.get('WR_MSG_EMBED_DESC_TP')} {result[9]}\n{self.bot.localisation.get('WR_MSG_EMBED_DESC_DYNO')} {float(result[10]):.3f}", value=f"\n", inline=False)
            embed.add_field(name=f"{self.bot.localisation.get('WR_MSG_EMBED_DESC_WR')} {float(result[11]):.3f}", value=f"**{self.bot.localisation.get('WR_MSG_EMBED_DESC_DRIVER')} {escaped_text}{result[14]}\n**", inline=False)
            embed.add_field(name=f"{self.bot.localisation.get('WR_MSG_EMBED_DESC_UPGRADES')} {result[12]}", value=f"**[{self.bot.localisation.get('WR_MSG_EMBED_DESC_SHIFT')}]({result[13]})**", inline=False)
