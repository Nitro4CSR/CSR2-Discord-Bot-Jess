...
class WRCommandCog(commands.Cog):
    ...
    @app_commands.command(name=localisation.get('WR_CMD_NAME'), description=localisation.get('WR_CMD_DESC'))
    ...
    async def wr(self, interaction: discord.Interaction, car: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
        await interaction.response.defer()
        try:
            header = localisation.get('WR_LOG_HEADER')
            logger.info(f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('WR_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}")
            log = f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('WR_CMD_NAME')} car: {car} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}"
    ...
    async def create_embeds(self, results: list, log: str):
        ...
        for row in results:
            ...
            embed.add_field(name=f"{localisation.get('WR_MSG_EMBED_DESC_UPGRADES')} {result[12]}", value=f"**[{localisation.get('WR_MSG_EMBED_DESC_SHIFT')}]({result[13]})**", inline=False)
