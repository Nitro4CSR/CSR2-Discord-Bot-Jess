...
class AddAdminCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        @app_commands.command(name=self.bot.localisation.get('ADDADMIN_CMD_NAME'), description=self.bot.localisation.get('ADDADMIN_CMD_DESC'))
        @app_commands.describe(user=self.bot.localisation.get('ADDADMIN_CMD_USER'))
        async def addadmin(interaction: discord.Interaction, user: discord.User):
            await interaction.response.defer(ephemeral=True)
            try:
                header = self.bot.localisation.get('ADDADMIN_LOG_HEADER')
                logger.info(f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('LISTADMIN_CMD_NAME')} user: {user}")
                log = f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('LISTADMIN_CMD_NAME')} user: {user}"
                ...
                if str(interaction.user.id) in admins or int(interaction.user.id) in await helpers.load_json_key("config", "ClientAdminIDs"):
                    ...
                    await interaction.followup.send(f"{user.mention} {self.bot.localisation.get('ADDADMIN_MSG_USER_ADDED')}", ephemeral=True)
        ...
        self.bot.tree.add_command(addadmin, guilds=[discord.Object(id=int(server)) for server in await helpers.load_json_key("config", "ClientAdminServers")])
