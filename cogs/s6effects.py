...
class S6ECog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        @app_commands.command(name=self.bot.localisation.get('S6_EFFECTS_CMD_NAME'), description=self.bot.localisation.get('S6_EFFECTS_CMD_DESC'))
        ...
        async def s6effects(interaction: discord.Interaction, car: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
            await interaction.response.defer()
            try:
                header = self.bot.localisation.get('S6_EFFECTS_LOG_HEADER')
                logger.info(f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('S6_EFFECTS_CMD_NAME')} car: {car}, rarity: {rarity}, tier: {tier} csr2_version: {csr2_version}")
                log = f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('S6_EFFECTS_CMD_NAME')} car: {car}, rarity: {rarity}, tier: {tier} csr2_version: {csr2_version}"
                                    ...
                                    async def button_callback(interaction: discord.Interaction, e = entry, log = log):
                                        ...
                                        try:
                                            ...
                                            if results:
                                                ...
                                                logger.info(f"{header}{self.bot.localisation.get('LOG_ERROR_NO_ENTRY')} {self.bot.localisation.get('S6_EFFECTS_LOG_CONTRIBUTE_NOTICE')}")
                                                log += f"\n{header}{self.bot.localisation.get('LOG_ERROR_NO_ENTRY')} {self.bot.localisation.get('S6_EFFECTS_LOG_CONTRIBUTE_NOTICE')}"
                                                await interaction.followup.send(f"{self.bot.localisation.get('S6_EFFECTS_MSG_WARNING_CAR_NOT_FOUND')} [{self.bot.localisation.get('S6_EFFECTS_MSG_CONTRIBUTE')}](https://docs.google.com/spreadsheets/d/1pBamDQTOcWyJoUowrXM05Tj567UVAti1VA8zWsSlrhM/edit)")
        ...
        self.bot.tree.add_command(s6effects)
    ...
    async def create_embeds(self, results: list, log: str):
        ...
        for row in results:
            ...
            embed.add_field(name=f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5PP')} {result[5]}\n{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5EVO')} {result[6]}\n", value=f"", inline=False)
            embed.add_field(name=f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5NOS')} {result[7]}\n{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5FD')} {result[8]}\n{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5TP')} {result[9]}\n", value=f"**{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_S5DYYO')} {float(result[10]):.3f}**", inline=False)
            embed.add_field(name=f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_TOPS6')} {sorted_categories[0]} ({float(sorted_offsets[0]):+.3f}) ({float(sorted_values[0]):.3f})", value=f"​\n`"+f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_PART'):<{spacing+3}}"+f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_OFFSET'):<16}"[:16]+f"{self.bot.localisation.get('S6_EFFECTS_MSG_EMBED_DESC_TIME'):>6}"+f"\n{chart}`", inline=False)
