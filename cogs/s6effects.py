...
class S6ECog(commands.Cog):
    ...
    @app_commands.command(name=localisation.get('S6_EFFECTS_CMD_NAME'), description=localisation.get('S6_EFFECTS_CMD_DESC'))
    ...
    async def s6effects(self, interaction: discord.Interaction, car: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
        await interaction.response.defer()
        try:
            header = localisation.get('S6_EFFECTS_LOG_HEADER')
            logger.info(f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('S6_EFFECTS_CMD_NAME')} car: {car}, rarity: {rarity}, tier: {tier} csr2_version: {csr2_version}")
            log = f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('S6_EFFECTS_CMD_NAME')} car: {car}, rarity: {rarity}, tier: {tier} csr2_version: {csr2_version}"
                                ...
                                async def button_callback(interaction: discord.Interaction, e = entry, log = log):
                                    ...
                                    try:
                                        ...
                                        if results:
                                            ...
                                            logger.info(f"{header}{localisation.get('LOG_ERROR_NO_ENTRY')} {localisation.get('S6_EFFECTS_LOG_CONTRIBUTE_NOTICE')}")
                                            log += f"\n{header}{localisation.get('LOG_ERROR_NO_ENTRY')} {localisation.get('S6_EFFECTS_LOG_CONTRIBUTE_NOTICE')}"
                                            await interaction.followup.send(f"{localisation.get('S6_EFFECTS_MSG_WARNING_CAR_NOT_FOUND')} [{localisation.get('S6_EFFECTS_MSG_CONTRIBUTE')}](https://docs.google.com/spreadsheets/d/1pBamDQTOcWyJoUowrXM05Tj567UVAti1VA8zWsSlrhM/edit)")
