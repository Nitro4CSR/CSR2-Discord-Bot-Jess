...
class WRlistCog(commands.Cog):
    ...
    async def cog_load(self):
        ...
        @app_commands.command(name=self.bot.localisation.get('WRLIST_CMD_NAME'), description=self.bot.localisation.get('WRLIST_CMD_DESC'))
        @app_commands.describe(car=self.bot.localisation.get('ANY_CMD_CAR'), rarity=self.bot.localisation.get('ANY_CMD_RARITY'), tier=self.bot.localisation.get('WRLIST_CMD_RACE'), csr2_version=self.bot.localisation.get('ANY_CMD_CSR2_VERSION'))
        @app_commands.choices(race=[app_commands.Choice(name=self.bot.localisation.get('WRLIST_CMD_RACE_OPTION_12_MILE'), value="'T4', 'T5'"), app_commands.Choice(name=self.bot.localisation.get('WRLIST_CMD_RACE_OPTION_14_MILE'), value="'T1', 'T2', 'T3'")])
        ...
        async def wrlist(interaction: discord.Interaction, car: str = None, race: str = None, rarity: str = None, tier: str = None, csr2_version: str = None):
            await interaction.response.defer()
            try:
                header = self.bot.localisation.get('WRLIST_LOG_HEADER')
                logger.info(f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('WRLIST_CMD_NAME')} car: {car} race: {race} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}")
                log = f"{header}{self.bot.localisation.get('LOG_CMD_TRIGGERED')} /{self.bot.localisation.get('WRLIST_CMD_NAME')} car: {car} race: {race} rarity: {rarity} tier: {tier} csr2_version: {csr2_version}"
                ...
                if results:
                    ...
                    if t1_t3_results and t4_t5_results:
                        ...
                        await interaction.followup.send(f"**{self.bot.localisation.get('WRLIST_MSG_14M_RESULTS')}**", embed=await view1.get_embed_page(), view=view1)
                        await interaction.followup.send(f"**{self.bot.localisation.get('WRLIST_MSG_12M_RESULTS')}**", embed=await view2.get_embed_page(), view=view2)
                    else:
                    ...
                    await interaction.followup.send(f"{self.bot.localisation.get('WRLIST_MSG_NO_RESULTS')}", ephemeral=True)
        ...
        self.bot.tree.add_command(wrlist)
    ...
    class PaginatedView(discord.ui.View):
        ...
        async def get_embed_page(self):
            ...
            car_display = self.car if self.car else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_ANY_CAR')}"
            race_display = (f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_12M')}" if self.race == "'T4', 'T5'" else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_14M')}") if self.race else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_ANY')}"
            rarity_display = f"{await helpers.emojify_rarity(self.rarity)}" if self.rarity else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_ANY_RARITY')}"
            tier_display = f"{await helpers.emojify_tier(self.tier)}" if self.tier else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_ANY_TIER')}"
            version_display = f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_VERSION')} {self.csr2_version}" if self.csr2_version else f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_ANY_VERSION')}"
    
            logger.info(f"{header}{self.bot.localisation.get('WRLIST_LOG_EMBED_PAGE')} {self.page_number}")
            embed = discord.Embed(
                title=f"{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_1')} {tier_display} {car_display} {self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_2')} {rarity_display}{self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_3')} {race_display} {self.bot.localisation.get('WRLIST_MSG_EMBED_TITLE_4')} {version_display}",
                description="\n".join(
