...
class CustomSQLCog(commands.Cog):
    ...
    @app_commands.command(name=localisation.get('CUSTOMSQL_CMD_NAME'), description=localisation.get('CUSTOMSQL_CMD_DESC'))
    @app_commands.describe(database=localisation.get('CUSTOMSQL_CMD_DATABASE'), select=localisation.get('CUSTOMSQL_CMD_SELECT'), update=localisation.get('CUSTOMSQL_CMD_UPDATE'), create_table=localisation.get('CUSTOMSQL_CMD_CREATE_TABLE'), insert_into=localisation.get('CUSTOMSQL_CMD_INSERT_INTO'), from_=localisation.get('CUSTOMSQL_CMD_FROM'), set_=localisation.get('CUSTOMSQL_CMD_SET'), where=localisation.get('CUSTOMSQL_CMD_WHERE'))
    ...
    async def customsql(self, interaction: discord.Interaction, database: app_commands.Choice[str], select: str = None, update: str = None, create_table: str = None, insert_into: str = None, from_: app_commands.Choice[str] = None, set_: str = None, where: str = None):
        ...
        try:
            ...
            logger.info(f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('CLIENTINFO_CMD_NAME')} database: {database} select: {select} update: {update} create_table: {create_table} insert_into: {insert_into} from_: {from_} set_: {set_} where: {where}")
            log = f"{header}{localisation.get('LOG_CMD_TRIGGERED')} /{localisation.get('CLIENTINFO_CMD_NAME')} database: {database} select: {select} update: {update} create_table: {create_table} insert_into: {insert_into} from_: {from_} set_: {set_} where: {where}"

            if any([update, create_table, insert_into, set_]):
                logger.info(f"{header}{localisation.get('CUSTOMSQL_LOG_PRIVELEGED_QUERY')}")
                log += f"\n{header}{localisation.get('CUSTOMSQL_LOG_PRIVELEGED_QUERY')}"
                ...
                if str(interaction.user.id) not in admins or int(interaction.user.id) not in await helpers.load_json_key("config", "ClientAdminIDs"):
                    ...
                    await interaction.response.send_message(f"{localisation.get('ADMIN_MSG_NO_PERMISSION')}", ephemeral=True)
            ...
            if results:
                ...
                await interaction.response.send_message(f"{localisation.get('CUSTOMSQL_MSG_DONE_EXECUTION_NO_RESULTS')}", ephemeral=True)
            await in_app_logging.send_log(self.bot, log, 2, 1, interaction)
    ...
    async def create_embeds(self, results: list, selections: list, log: str):
        ...
        for result in results:
            ...
            if len(desc) > 4096:
                ...
                desc1 = desc
            embed = discord.Embed(
                title=f"{localisation.get('CUSTOMSQL_MSG_EMBED_TITLE')}",
                description=desc1,
            ...
            try:
                ...
                try:
                    ...
                    logger.info(f"{header}{localisation.get('CUSTOMSQL_LOG_SPLIT_EMBED_NOT_NEEDED_2')}")
                    log += f"\n{header}{localisation.get('CUSTOMSQL_LOG_SPLIT_EMBED_NOT_NEEDED_2')}"
            except:
                logger.info(f"{header}{localisation.get('CUSTOMSQL_LOG_SPLIT_EMBED_NOT_NEEDED_1')}")
                log += f"\n{header}{localisation.get('CUSTOMSQL_LOG_SPLIT_EMBED_NOT_NEEDED_1')}"
            embed.set_footer(text=f"{localisation.get('CUSTOMSQL_MSG_EMBED_DESC_FOOTER')} {idx} / {len(results)}")
